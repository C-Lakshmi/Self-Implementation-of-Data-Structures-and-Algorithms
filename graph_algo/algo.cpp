    ...
    for (int i=0;i<n-1;i++)
    {
        for (int j=1;j<=n;j++)
        {
            for (int k=0;k<v[j].size();k++)
                dis[v[j][k].first]=min_num(dis[v[j][k].first],dis[j]+v[j][k].second);
        }
    }
    cout<<"Shortest Path from "<<s<<" to all other nodes: "<<endl;
    for (int i=1;i<=n;i++) cout<<"Node "<<i<<": "<<dis[i]<<endl;
...
void FloydWarshall(vector <pair < int,int> > v[],int n,int s)
{
    ...
    for (int k = 1; k <= n; k++) {
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= n; j++) {
                a[i][j] = min_num(a[i][j],a[i][k]+a[k][j]);
            }}}
...
#include "time.h"
    ...
    while(1)
    {
        ...
        cout<<"Enter a different graph and calculate shortest path? (Y/N)"<<endl;
        char x;
        cin>>x;
        if (x=='N') break;
    }

    cout<<"Perform time taken vs input size comparison for different algorithms? (Y/N)"<<endl;
    char x;
    cin>>x;
