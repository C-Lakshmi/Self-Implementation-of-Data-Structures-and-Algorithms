    ...
    for (int i=0;i<n-1;i++)
    {
        for (int j=1;j<=n;j++)
        {
            for (int k=0;k<v[j].size();k++)
                dis[v[j][k].first]=min_num2(dis[v[j][k].first],dis[j]+v[j][k].second);
        }
    }
...
void FloydWarshall2(int n,int s)
{
    ...
    for (int k = 1; k <= n; k++) {
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= n; j++) {
                a[i][j] = min_num2(a[i][j],a[i][k]+a[k][j]);
            }}}
...
void time_analysis()
{
    ...
    for (int z=0;z<4;z++)
    {
        ...
        b.push_back(Time1*10e3);
        d.push_back(Time2*10e3);
        f.push_back(Time3*10e3);
